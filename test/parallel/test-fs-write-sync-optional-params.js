...

{
  ...
  for (const badBuffer of [
    ...
    testInvalid(dest, 'ERR_INVALID_ARG_TYPE', common.mustNotMutate(badBuffer));
  }
  ...
  testInvalid(dest, 'ERR_OUT_OF_RANGE', buffer, common.mustNotMutate({ length: 5 }));
  testInvalid(dest, 'ERR_OUT_OF_RANGE', buffer, common.mustNotMutate({ offset: 5 }));
  testInvalid(dest, 'ERR_OUT_OF_RANGE', buffer, common.mustNotMutate({ length: 1, offset: 3 }));
  testInvalid(dest, 'ERR_OUT_OF_RANGE', buffer, common.mustNotMutate({ length: -1 }));
  testInvalid(dest, 'ERR_OUT_OF_RANGE', buffer, common.mustNotMutate({ offset: -1 }));
  testInvalid(dest, 'ERR_INVALID_ARG_TYPE', buffer, common.mustNotMutate({ offset: false }));
  testInvalid(dest, 'ERR_INVALID_ARG_TYPE', buffer, common.mustNotMutate({ offset: true }));
  ...
  for (const options of [
    ...
    testValid(dest, buffer, common.mustNotMutate(options));
  }
}
