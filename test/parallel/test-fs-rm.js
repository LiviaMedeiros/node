...
function gitInit(gitDirectory) {
  ...
  execSync('git init', common.mustNotMutate({ cwd: gitDirectory }));
}

function makeNonEmptyDirectory(depth, files, folders, dirname, createSymLinks) {
  fs.mkdirSync(dirname, common.mustNotMutate({ recursive: true }));
  fs.writeFileSync(path.join(dirname, 'text.txt'), 'hello', 'utf8');

  const options = common.mustNotMutate({ flag: 'wx' });
...
function removeAsync(dir) {
  ...
  fs.rm(dir, common.mustCall((err) => {
    ...
    fs.rm(dir, common.mustNotMutate({ recursive: false }), common.mustCall((err) => {
      assert.strictEqual(err.syscall, 'rm');

      // Recursive removal should succeed.
      fs.rm(dir, common.mustNotMutate({ recursive: true }), common.mustSucceed(() => {
...
// Test the asynchronous version
{
  ...
  fs.rm(
    ...
    common.mustNotMutate({ recursive: true }),
    common.mustCall((err) => {
  ...
  fs.rm(filePath, common.mustNotMutate({ recursive: true }), common.mustCall((err) => {
    try {
      assert.strictEqual(err, null);
      assert.strictEqual(fs.existsSync(filePath), false);
    } finally {
      fs.rmSync(filePath, common.mustNotMutate({ force: true }));
    }
  }));
}
...
if (isGitPresent) {
  ...
  fs.rm(gitDirectory, common.mustNotMutate({ recursive: true }), common.mustSucceed(() => {
...
// Test the synchronous version.
{
  ...
  assert.throws(() => {
    fs.rmSync(dir, common.mustNotMutate({ recursive: false }));
  }, { syscall: 'rm' });

  // Should fail if target does not exist
  assert.throws(() => {
    fs.rmSync(path.join(tmpdir.path, 'noexist.txt'), common.mustNotMutate({ recursive: true }));
  }, {
    code: 'ENOENT',
    name: 'Error',
  ...
  try {
    fs.rmSync(filePath, common.mustNotMutate({ recursive: true }));
  } finally {
    fs.rmSync(filePath, common.mustNotMutate({ force: true }));
  }
  ...
  try {
    fs.rmSync(fileURL, common.mustNotMutate({ recursive: true }));
  } finally {
    fs.rmSync(fileURL, common.mustNotMutate({ force: true }));
  }
...
if (isGitPresent) {
  const gitDirectory = nextDirPath();
  gitInit(gitDirectory);
  fs.rmSync(gitDirectory, common.mustNotMutate({ recursive: true }));
  assert.strictEqual(fs.existsSync(gitDirectory), false);
}
...
(async () => {
  ...
  await assert.rejects(fs.promises.rm(dir, common.mustNotMutate({ recursive: false })), {
    syscall: 'rm'
  });

  // Recursive removal should succeed.
  await fs.promises.rm(dir, common.mustNotMutate({ recursive: true }));
  ...
  await fs.promises.rm(path.join(tmpdir.path, 'noexist.txt'), common.mustNotMutate({ force: true }));
  ...
  try {
    await fs.promises.rm(filePath, common.mustNotMutate({ recursive: true }));
  } finally {
    fs.rmSync(filePath, common.mustNotMutate({ force: true }));
  }
  ...
  try {
    await fs.promises.rm(fileURL, common.mustNotMutate({ recursive: true }));
  } finally {
    fs.rmSync(fileURL, common.mustNotMutate({ force: true }));
  }
...
if (isGitPresent) {
  (async () => {
    const gitDirectory = nextDirPath();
    gitInit(gitDirectory);
    await fs.promises.rm(gitDirectory, common.mustNotMutate({ recursive: true }));
    assert.strictEqual(fs.existsSync(gitDirectory), false);
  })().then(common.mustCall());
}
  ...
  if (!common.isIBMi && (common.isWindows || process.getuid() !== 0)) {
...

    {
      ...
      try {
        ...
        try {
          assert.throws(() => {
            fs.rmSync(root, common.mustNotMutate({ recursive: true }));
          }, {
            code,
            name: 'Error',
          });
