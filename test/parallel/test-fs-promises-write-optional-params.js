...
(async () => {
  ...
  for (const badBuffer of [
    ...
    await testInvalid(dest, 'ERR_INVALID_ARG_TYPE', common.mustNotMutate(badBuffer), common.mustNotMutate({}));
  }
  ...
  await testInvalid(dest, 'ERR_OUT_OF_RANGE', buffer, common.mustNotMutate({ length: 5 }));
  await testInvalid(dest, 'ERR_OUT_OF_RANGE', buffer, common.mustNotMutate({ offset: 5 }));
  await testInvalid(dest, 'ERR_OUT_OF_RANGE', buffer, common.mustNotMutate({ length: 1, offset: 3 }));
  await testInvalid(dest, 'ERR_OUT_OF_RANGE', buffer, common.mustNotMutate({ length: -1 }));
  await testInvalid(dest, 'ERR_OUT_OF_RANGE', buffer, common.mustNotMutate({ offset: -1 }));
  await testInvalid(dest, 'ERR_INVALID_ARG_TYPE', buffer, common.mustNotMutate({ offset: false }));
  await testInvalid(dest, 'ERR_INVALID_ARG_TYPE', buffer, common.mustNotMutate({ offset: true }));
  ...
  for (const options of [
    ...
    await testValid(dest, buffer, common.mustNotMutate(options));
  }
