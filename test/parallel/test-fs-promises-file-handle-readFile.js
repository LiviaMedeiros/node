...
async function doReadAndCancel() {
  // Signal aborted from the start
  {
    ...
    try {
      ...
      await assert.rejects(readFile(fileHandle, common.mustNotMutate({ signal })), {
        name: 'AbortError'
      });
    } finally {
      await fileHandle.close();
  ...
  // Signal aborted right before buffer read
  {
    ...
    await assert.rejects(fileHandle.readFile(common.mustNotMutate({ signal, encoding: 'utf8' })), {
      name: 'AbortError'
    }, 'tick-1');
