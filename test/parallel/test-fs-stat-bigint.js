...
const runSyncTest = (func, arg) => {
  ...
  const bigintStats = func(arg, common.mustNotMutate({ bigint: true }));
...

{
  assert.throws(
    ...
    { code: 'ENOENT' });
  assert.strictEqual(
    fs.statSync('does_not_exist', common.mustNotMutate({ throwIfNoEntry: false })),
    undefined);
}

{
  assert.throws(
    ...
    { code: 'ENOENT' });
  assert.strictEqual(
    fs.lstatSync('does_not_exist', common.mustNotMutate({ throwIfNoEntry: false })),
    undefined);
}

{
  assert.throws(
    ...
    { code: 'EBADF' });
  assert.throws(
    () => fs.fstatSync(9999, common.mustNotMutate({ throwIfNoEntry: false })),
    { code: 'EBADF' });
}

const runCallbackTest = (func, arg, done) => {
  const startTime = process.hrtime.bigint();
  func(arg, common.mustNotMutate({ bigint: true }), common.mustCall((err, bigintStats) => {
...
const runPromiseTest = async (func, arg) => {
  const startTime = process.hrtime.bigint();
  const bigintStats = await func(arg, common.mustNotMutate({ bigint: true }));
...
(async function() {
  ...
  const startTime = process.hrtime.bigint();
  const bigintStats = await handle.stat(common.mustNotMutate({ bigint: true }));
