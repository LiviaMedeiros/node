import { mustCall, mustNotMutate } from '../common/index.mjs';
...
// It copies a nested folder structure with files and folders.
{
  ...
  const dest = nextdir();
  cpSync(src, dest, mustNotMutate({ recursive: true }));
...
{
  const src = nextdir();
  mkdirSync(join(src, 'a', 'b'), mustNotMutate({ recursive: true }));
  writeFileSync(join(src, 'README.md'), 'hello world', 'utf8');
  const dest = nextdir();
  cpSync(src, dest, mustNotMutate({ recursive: true }));
  const initialStat = lstatSync(join(dest, 'README.md'));
  cpSync(src, dest, mustNotMutate({ force: false, recursive: true }));
...
// It overwrites existing files if force is true.
{
  const src = './test/fixtures/copy/kitchen-sink';
  const dest = nextdir();
  mkdirSync(dest, mustNotMutate({ recursive: true }));
...
// when dereference is true, and force is false (fails silently).
{
  ...
  cpSync(src, dest, mustNotMutate({ dereference: true, recursive: true }));
  cpSync(src, dest, mustNotMutate({ dereference: true, recursive: true }));
...
{
  const src = nextdir();
  mkdirSync(src, mustNotMutate({ recursive: true }));
  ...
  cpSync(join(src, 'bar.js'), destFile, mustNotMutate({ dereference: true, recursive: true }));
  const stat = lstatSync(destFile);
  assert(stat.isFile());
}
...
// It throws an error when both dereference and verbatimSymlinks are enabled.
{
  const src = './test/fixtures/copy/kitchen-sink';
  assert.throws(
    () => cpSync(src, src, mustNotMutate({ dereference: true, verbatimSymlinks: true })),
...
// It resolves relative symlinks when verbatimSymlinks is false.
{
  ...
  cpSync(src, dest, mustNotMutate({ recursive: true, verbatimSymlinks: false }));
  const link = readlinkSync(join(dest, 'bar.js'));
  assert.strictEqual(link, join(src, 'foo.js'));
}
...
// It does not resolve relative symlinks when verbatimSymlinks is true.
{
  ...
  cpSync(src, dest, mustNotMutate({ recursive: true, verbatimSymlinks: true }));
  const link = readlinkSync(join(dest, 'bar.js'));
...
// It throws error if symlink in dest points to location in src.
{
  ...
  const dest = nextdir();
  mkdirSync(join(dest, 'a'), mustNotMutate({ recursive: true }));
...
{
  const src = nextdir();
  mkdirSync(join(src, 'a'), mustNotMutate({ recursive: true }));
  const dest = nextdir();
...
// It allows file to be copied to a file path.
{
  ...
  cpSync(srcFile, destFile, mustNotMutate({ dereference: true }));
  const stat = lstatSync(destFile);
  assert(stat.isFile());
}
...
// It copies timestamps from src to dest if preserveTimestamps is true.
{
  const src = './test/fixtures/copy/kitchen-sink';
  const dest = nextdir();
  cpSync(src, dest, mustNotMutate({ preserveTimestamps: true, recursive: true }));
  assertDirEquivalent(src, dest);
...
// It makes file writeable when updating timestamp, if not writeable.
{
  ...
  const dest = nextdir();
  mkdirSync(dest, mustNotMutate({ recursive: true }));
  writeFileSync(join(src, 'foo.txt'), 'foo', mustNotMutate({ mode: 0o444 }));
  cpSync(src, dest, mustNotMutate({ preserveTimestamps: true, recursive: true }));
  assertDirEquivalent(src, dest);
...
// It accepts file URL as src and dest.
{
  const src = './test/fixtures/copy/kitchen-sink';
  const dest = nextdir();
  cpSync(pathToFileURL(src), pathToFileURL(dest), mustNotMutate({ recursive: true }));
  assertDirEquivalent(src, dest);
}
...
// It copies a nested folder structure with files and folders.
{
  const src = './test/fixtures/copy/kitchen-sink';
  const dest = nextdir();
  cp(src, dest, mustNotMutate({ recursive: true }), mustCall((err) => {
...
// It copies file itself, rather than symlink, when dereference is true.
{
  ...
  cp(join(src, 'bar.js'), destFile, mustNotMutate({ dereference: true }),
     mustCall((err) => {
       assert.strictEqual(err, null);
       const stat = lstatSync(destFile);
       assert(stat.isFile());
...
// It allows file to be copied to a file path.
{
  ...
  const destFile = join(nextdir(), 'index.js');
  cp(srcFile, destFile, mustNotMutate({ dereference: true }), mustCall((err) => {
    assert.strictEqual(err, null);
    const stat = lstatSync(destFile);
    assert(stat.isFile());
  }));
...
// It accepts file URL as src and dest.
{
  const src = './test/fixtures/copy/kitchen-sink';
  const dest = nextdir();
  cp(pathToFileURL(src), pathToFileURL(dest), mustNotMutate({ recursive: true }),
     mustCall((err) => {
       assert.strictEqual(err, null);
       assertDirEquivalent(src, dest);
     }));
...
// It copies a nested folder structure with files and folders.
{
  const src = './test/fixtures/copy/kitchen-sink';
  const dest = nextdir();
  const p = await fs.promises.cp(src, dest, mustNotMutate({ recursive: true }));
...
// It allows async error to be caught.
{
  const src = './test/fixtures/copy/kitchen-sink';
  const dest = nextdir();
  await fs.promises.cp(src, dest, mustNotMutate({ recursive: true }));
...
function collectEntries(dir, dirEntries) {
  const newEntries = readdirSync(dir, mustNotMutate({ withFileTypes: true }));
